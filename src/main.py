...
def parse_vrc_instance_id(instance_id):
    ...
    for opt in parts[1:]:
        if opt == "canRequestInvite":
            ...
            # For cases like ~friends or ~private without parentheses if they exist
            internal_type = opt

    # Map internal types to user-friendly VRChat display names
    type_map = {
        "public": "Public",
        "hidden": "Friend+",
        "friends": "Friends",
    ...
    # Map region codes to Japanese names
    region_map = {
        "jp": "日本",
        "us": "アメリカ西",
        "use": "アメリカ東",
    ...
    try:
        with Image.open(file_path) as img:
            ...
            if vrchat_info or vrcx_info:
                ...
                if display_date != "N/A":
                    # Normalize sub-seconds for fromisoformat
                ...
                if instance_id:
                    ...
                    if parsed:
                        print(
                            f"インスタンス: {parsed['instance_type']} #{parsed['instance_id']} / 地域: {parsed['region']}"
                        )
