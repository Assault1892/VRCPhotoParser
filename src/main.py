...
def extract_vrchat_xml_info(xml_str):
    ...
    try:
        ...
        for desc in descriptions:
            for key, tag in [("CreateDate", "{http://ns.adobe.com/xap/1.0/}CreateDate"),
                ...
                if not info.get(key):
                    val = desc.get(tag)
                    if val: info[key] = val
            for key, xpath in [("CreateDate", ".//xmp:CreateDate"),
                               ("Author", ".//xmp:Author"),
                               ("WorldID", ".//vrc:WorldID"),
                ...
                if not info.get(key):
                    ...
                    if el is not None and el.text: info[key] = el.text
...
def parse_vrc_instance_id(instance_id):
    ...
    if not instance_id or ":" not in instance_id: return None
    parts = instance_id.split("~")
    base_part = parts[0].split(":")
    res = {"world_id": base_part[0], "instance_id": base_part[1] if len(base_part) > 1 else "N/A",
           "owner_id": None, "group_id": None, "region": "us"}
    internal_type, group_access_type, can_request_invite = "public", None, False
    for opt in parts[1:]:
        if opt == "canRequestInvite": can_request_invite = True
        elif opt.startswith("region("): res["region"] = opt[7:-1]
        elif opt.startswith("group("): res["group_id"] = opt[6:-1]
        elif opt.startswith("groupAccessType("): group_access_type = opt[16:-1]
        elif "(" in opt and opt.endswith(")"):
            idx = opt.find("(")
            internal_type, res["owner_id"] = opt[:idx], opt[idx+1:-1]
        else: internal_type = opt
    if res["group_id"]:
        group_map = {"public": "Group Public", "plus": "Group+", "members": "Group Only"}
        res["instance_type"] = group_map.get(group_access_type, f"Group ({group_access_type})")
    else:
...
def print_cli_output(data):
    ...
    if not info:
        ...
        return
    print(f"--- {t['photo_info_header']} ---")
    if info["datetime"] != "N/A": print(f"{t['datetime']}: {info['datetime']}")
